...
export class Target {
  ...
  constructor(private word: string, private manager: TargetManager) {
    ...
    const letters: string[] = shuffle(word.split(""));
  ...
  checkSubmission(word: string): boolean {
    ...
    for (const [letter, count] of Object.entries(bins)) {
...
export class TargetManager {
  ...
  constructor() {
    const data: string[][] = JSON.parse(readFileSync(targetsPath, "utf-8"));
  ...
  getTargets(): Target[] | null {
    const word3 = this.pool_three.pop();
    const words: (string[] | undefined)[] = word3 ? [word3] : [];
